...
Phase 1 완료: **Q.2 포렌식(원시 디스크 스캔, 야간 CI)**, **Q.1 E2E 하네스(A~E 전부, `open.rs` 열기·저장 단계, ADR-0007 수신자 재봉인, S6 열기 UI `session.rs`)**, **C 트랙 전부**(컨테이너 v1.2, 재봉인 버전 체인, 24h 퍼징 420,679,318 runs·무크래시, 복구 코드 백업), **A 트랙 대부분**(`zbacs-auth`: AuthProvider·이중 경로·확인 정책 T23·OS 키저장소·OTAK, Windows Hello/TPM 코드는 크로스컴파일까지), **H 트랙**(FileRegistry/AccessPolicy/AuditLog/P256Validator, `zbacs-chain` 이벤트 감시·오프라인 캐시, **UUPS+Timelock 배포·주소 파일**, **H.8a Rust 스마트계정·번들러 클라이언트(스파이크 벡터 바이트 일치)**), **R 트랙**(axum Relay, 장애 조치·멱등 재시도 클라이언트, **Docker 이미지·셀프호스팅 문서 R.4**), **G 트랙**(세션 상태머신, 보호 작업공간, 열람 앱 실행·저장 감지, Agent 골격, **첫 실행 온보딩**, **잠그기 UI**, **열람 요청·대기 UI(G.9)**, **데스크톱 승인 UI(G.10, 시나리오 B 왕복 테스트)**, **회수·허락 목록(G.11)**, **기록 화면(G.12 로컬, 체인 병합은 H.8 뒤)**), **U 트랙**(디자인 토큰, 온보딩 흐름, 용어 린트, 승인 방식 선택, 봉인 다이얼로그, **오류 카탈로그 U.4**, **알림 액션 버튼 U.5(Windows 실기 확인 대기)**).

**베타까지 남은 것은 `docs/dev_plan.md` §1.9 "베타 컷"에 정리돼 있다** — 코드 0태스크 — Q.1 E2E 하네스가 **A~E 전부**를 검사한다(G.8 수신자 재봉인 완결, ADR-0007). **열기 UI(S6)도 배선 완료**(`session.rs`: 열기→앱 실행→저장 감지→재봉인/폐기→wipe). 남은 코드는 H.8(체인 쓰기; G.12 체인 병합·bumpVersion 포함)과, 사용자만 할 수 있는 Windows 실기 확인. 후자가 ◐ 8개를 한꺼번에 막고 있는 가장 큰 일정 리스크다.
...
다음: `Z-1.H.8` b단계 — Agent 배선(`zbacs-chain::{aa,calls,bundler}`는 a단계로 완료·벡터 검증됨): 첫 실행에서 소유자 Kernel 계정 주소 계산(`account_on_chain` pending 해소), 잠글 때 `register`, 허락 시 `grant` UserOp → `GrantMsg.tx_hash`, 버전 통지 수락 시 `bumpVersion`, 회수 시 `revoke`, G.12에 체인 이벤트 병합. 실제 전송에는 테스트넷 배포(H.4 스크립트, 배포 키·faucet = 사용자)·`Z-1.H.11` 주소 내장·Pimlico 키(H.9)가 필요하므로, Anvil + 직접 전송(EOA) 경로로 먼저 E2E를 만든다. Relay 호스팅(계정)은 사용자 차례 — `docs/relay_selfhost.md` §5. 열기(G.7/G.8)는 승인 후 `Held.envelope`(aad=grantId)를 여는 것부터. Windows 실기 확인(`docs/windows_checklist.md` §0-A 설치 파일 경로, §1~§4, 특히 **§3.5 첫 실행 2탭**, **§3.6 잠그기**)은 사용자 차례.
...
docker compose up -d && curl -fsS localhost:8787/v1/health   # Relay 컨테이너 (Z-1.R.4)
...
tools/forensic.sh [image|dir]              # 세션 후 평문 잔존 스캔 (image=루프 디스크 원시 스캔, sudo 필요; 야간 CI)
```
