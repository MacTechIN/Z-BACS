...
# Windows 전용 코드 컴파일 검증 (Linux에서). zbacs-relay-client는 제외 — rustls/ring이
# Windows용 C 크로스 툴체인을 요구하고, Windows 코드가 한 줄도 없어 얻을 게 없다.
# 실제 Windows 빌드는 CI의 windows-latest 러너가 담당한다.
cargo check --target x86_64-pc-windows-gnu --workspace --exclude zbacs-relay-client
