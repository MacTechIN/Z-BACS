...
| 결과 | **성공** — 소프트웨어 패스키 서명 1건으로 Kernel v3.1 계정 배포 + 호출 실행이 실제 EntryPoint v0.7 `handleOps`를 통과 |
...
- 번들러 제출(`--send`)은 `PIMLICO_API_KEY` 필요 — 스파이크에서는 미실행. 포크에서 EntryPoint를 직접 호출한 것이 실제 번들러 경로와 다른 점은 mempool 검증 규칙(ERC-7562 opcode 제한)뿐이며, Kernel v3.1 + WebAuthn 검증기는 Pimlico가 공식 지원한다.

## 4. Phase 1로 넘기는 것
- `Z-1.H.8`: `usePrecompiled` 제어, 페이마스터(Pimlico 샌드박스) 연결, 실제 번들러 제출 1건.
