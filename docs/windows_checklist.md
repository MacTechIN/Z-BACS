...
| 문서 버전 | 1.6 (2026-09-22: §3.9 허락 거두기 추가) |
|---|---|
...
### 3.7 주인에게 물어보기 (Z-1.G.9)

Relay가 필요하다. 호스팅 전이므로 같은 PC에서 `cargo run -p zbacs-relay`를 띄우고 Agent를 `ZBACS_RELAY_URL=http://127.0.0.1:8787`로 실행한다(설치 파일은 기본값이 이 주소라 그대로 된다).

1. §3.6에서 잠근 `.zbacs`를 두 번 누른다 → 홈 카드에 [주인에게 물어보기]가 활성이어야 한다.
2. 누르면 진행 링 + "주인의 허락을 기다리는 중…" + [취소]. Relay 터미널에 요청 1건이 찍힌다.
3. 아무 답도 주지 않고 2분 → 문구가 "아직 답이 없어요. 계속 기다리고 있어요."로 바뀌고, 5분 → "주인이 아직 답하지 않았어요" + [다시 물어보기].
4. Relay를 끄고 다시 누르면 → "지금은 주인에게 연결할 수 없어요…" (멈추지 않고 즉시).
5. [취소] → 홈으로 돌아온다.

### 3.8 허락하기 (Z-1.G.10) — PC 한 대로

같은 Agent가 주인이자 받은 사람이 된다(같은 계정이므로 요청이 자기 받은편지함으로 온다).
1. §3.6에서 잠근 파일을 두 번 누르고 [주인에게 물어보기].
2. 3초 안에 창이 앞으로 오고 S5 화면: `"<파일명>" 파일을 열려고 해요.` + "읽기만 하고 싶어 해요." + [읽기만 허락] [편집도 허락] [거절]. 파일명이 §3.6에서 잠근 이름과 같아야 한다.
3. [편집도 허락] → **Windows Hello 창이 떠야 한다**(편집은 항상 OS 확인, T23). 통과하면 "허락했어요" → 기다리던 쪽도 "허락받았어요"로 바뀐다.
4. 다시 요청 → [읽기만 허락] → "이 기기에서 바로 승인"을 골랐다면 Hello 창 없이 바로, "얼굴이나 지문으로"를 골랐다면 Hello 창.
5. [거절] → "거절했어요", 요청 쪽은 "주인이 허락하지 않았어요".
6. 다른 PC(또는 §3.6을 안 거친 계정)에서 잠근 파일로 요청하면 "이 컴퓨터에서 잠근 기록이 없는 파일이에요" + [거절]만.

### 3.9 허락 거두기 (Z-1.G.11) — PC 한 대로

1. §3.8에서 허락한 직후 홈에 [내가 허락한 파일]이 보여야 한다. 누르면 카드: 파일명, "읽기만 허락했어요.", "열 수 있음" 태그, 남은 시간, [허락 거두기].
2. [허락 거두기] → "허락을 거뒀어요". 목록에서 그 카드는 "거둠"으로 바뀌고 버튼이 사라진다.
3. 요청했던 쪽(같은 PC의 파일 카드) — 3초 안에 "주인이 허락을 거뒀어요" 화면 + 카드 태그 "거둬짐". [다시 물어보기]로 새 요청이 가능해야 한다.
4. 앱을 껐다 켜도 [내가 허락한 파일]에 "거둠" 기록이 남아 있어야 한다(`sealed.json`).
