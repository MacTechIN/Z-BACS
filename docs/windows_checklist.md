...
| 문서 버전 | 1.9 (2026-09-25: §3.12 열기 추가) |
|---|---|
...
1. 홈의 [기록] → §3.6~§3.9를 거쳤다면 위에서부터: "…허락을 거뒀어요" / "주인이 …의 허락을 거뒀어요" / "…읽기만 허락했어요" / "주인이 받은 파일을 읽기만 허락했어요" / "누군가 … 열고 싶어 했어요" / "받은 파일을 열려고 주인에게 물어봤어요" / "… 파일을 잠갔어요".
2. 칩 [거둠]을 누르면 거둠 두 줄만, [잠금]은 잠근 한 줄만 남아야 한다.
3. 앱을 껐다 켜도 같은 기록이 보여야 한다(`audit.jsonl`).

### 3.11 알림에서 바로 답하기 (Z-1.U.5, U-3)

설치 파일로 설치한 Agent(시작 메뉴 바로가기가 있어야 토스트가 뜬다)로 확인한다. `cargo tauri dev`로 띄운 개발 빌드는 토스트가 안 보일 수 있으니 `ZBACS_TOAST_POWERSHELL=1`을 주고 실행한다.
1. §3.7처럼 [주인에게 물어보기] → 3초 안에 **오른쪽 아래 토스트**: "누군가 내 파일을 열려고 해요" + `"<파일명>" 파일을 읽기만 하고 싶어 해요.` + 버튼 4개 [읽기만 허락] [편집도 허락] [거절] [앱에서 보기].
2. [편집도 허락] → Windows Hello 창 → 통과하면 토스트 "허락했어요" + 요청 쪽 화면이 "허락받았어요"로. **창을 한 번도 열지 않고** 끝나야 한다(U-3).
3. 다시 요청 → 토스트 [거절] → "거절했어요".
4. [앱에서 보기] → 창이 앞으로 오고 S5 화면.
5. 토스트를 무시하고 창을 열어 S5에서 답해도 토스트가 사라지거나 눌러도 "이 요청은 이미 처리됐어요"가 나와야 한다.
6. 알림 설정에서 Z-BACS 알림을 끈 상태 → 창이 앞으로 오는 것으로 대체되는지(§3.8).
안 되는 경우 붙여 줄 것: 토스트가 아예 없는지 / 버튼이 없는지 / 눌러도 반응이 없는지, 그리고 `설정 → 시스템 → 알림`에 Z-BACS가 있는지.

### 3.12 열기 (S6, Z-1.G.7/G.8)

1. §3.8에서 허락받은 카드의 [열기] → "파일이 열려 있어요" 화면(열린 금고, 남은 시간, [지금 잠그기]) + 파일 종류의 기본 앱(메모장/Word)이 뜬다. 열린 파일의 경로가 `%LOCALAPPDATA%\Z-BACS\sessions\…`인지 확인.
2. **읽기만** 허락: 앱에서 저장 시도 → 읽기전용이라 거부되거나 다른 이름으로 저장 안내. 앱을 닫으면 → "다시 잠겼어요". `sessions` 폴더가 비어 있어야 한다.
3. **편집** 허락: 앱에서 한 줄 고치고 저장 → 카드 태그가 "다시 잠그는 중" → "열림", 메모 "저장한 내용을 다시 잠갔고 주인에게 알렸어요". 주인 쪽 [내가 허락한 파일]에서 같은 파일이 그대로 보이고, 다시 요청하면 새 버전으로 허락된다(§3.8).
4. 앱을 열어 둔 채 [지금 잠그기] → 앱이 닫히고(닫기 요청 후 5초 뒤 강제) "다시 잠겼어요".
5. 앱을 열어 둔 채 주인 쪽에서 [허락 거두기] → 3초 안에 앱이 닫히고 "주인이 허락을 거뒀어요".
6. OS 기본 앱으로 열었을 때(추적 불가) 앱을 그냥 닫으면 화면이 바뀌지 않을 수 있다 — 이 경우 [지금 잠그기]로 끝내는지, 그리고 그 사실을 알려 줄 것.
