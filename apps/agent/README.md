...
허락받은 뒤 파일을 여는 것(Z-1.G.7/G.8)은 아직이다. 답 화면이 "파일 열기는 다음 단계에서 연결됩니다"라고 말하고 [열기] 버튼을 비활성으로 둔다. 소유자 서명 검증(`owner_signature_check`)은 소유자 승인 키를 체인에서 읽어야 해서 Z-1.H.8/H.10 뒤이며, 개발용 정보 패널의 `pending`에만 나온다.
